...
import cn.sf.project.aop.annos.AutoLogSkip;
...
public class AutoLogAop {
    ...
    @Pointcut("@within(cn.sf.project.aop.annos.AutoLog) && execution(public * *(..))")
    public void AutoLogAspectClass() {
    }

    //@annotation 用于匹配当前执行方法持有指定注解的方法；运行切入
    @Pointcut("@annotation(cn.sf.project.aop.annos.AutoLog)")
