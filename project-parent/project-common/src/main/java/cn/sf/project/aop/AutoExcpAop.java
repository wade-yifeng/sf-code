package cn.sf.project.aop;


import cn.sf.project.aop.annos.AutoExcp;
import cn.sf.project.aop.annos.AutoExcpSkip;
import cn.sf.project.aop.excps.IPrintErrorLog;
import cn.sf.project.aop.excps.IPrintInfoLog;
import cn.sf.project.aop.excps.IReThrowException;
import cn.sf.project.aop.excps.ServiceException;
...
public class AutoExcpAop {
    ...
    @Pointcut("@within(cn.sf.project.aop.annos.AutoExcp) && execution(public * *(..))")
    public void AutoExcpAspectClass() {
    }

    //@annotation 用于匹配当前执行方法持有指定注解的方法；运行切入
    @Pointcut("@annotation(cn.sf.project.aop.annos.AutoExcp)")
    ...
    public Object doAroundForExcp(final ProceedingJoinPoint pjp) {
        ...
        try {
            ...
            log.error("======================DataAccessException|SQLException======================");
    ...
    protected Object failReturn(String errorMessage, Class returnType){
        ...
        try {
            ...
            log.warn(returnType.getName()+"没有默认构造方法");
            log.debug("没有默认构造方法",ex);
            return null;
        }
    }
